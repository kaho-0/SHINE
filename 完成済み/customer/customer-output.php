...
<?php
    ...
    if(empty($sql->fetchAll())){
        if(isset($_SESSION['client'])){
            $sql=$pdo->prepare('update client set ID=?,PW=?,name=?,BD=?,mail=?,address=?,tell=?');
            $sql->execute([
                $_POST['ID'],$_POST['PW'],$_POST['name'],$_POST['BD'],$_POST['address'],$_POST['mail'],$_POST['tell']]);
            $_SESSION['client']=[
                'ID'=>$ID,'PW'=>$_POST['PW'], 'name'=>$_POST['name'],'BD'=>$_POST['BD'],
                'mail'=>$_POST['mail'],'address'=>$_POST['address'], 'tell'=>$_POST['tell']
                ];
            echo 'お客様情報を更新しました。';
        }else{
